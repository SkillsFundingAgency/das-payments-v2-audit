...
using SFA.DAS.Payments.Audit.ArchiveService.EntityTrigger;
...
namespace SFA.DAS.Payments.Audit.ArchiveService.Helpers
{
    public class EntityHelper : IEntityHelper
    {
        ...
        public async Task ClearCurrentStatus(DurableTaskClient client, StatusHelper.EntityState state)
        {
        ...
        public async Task UpdateCurrentJobStatus(DurableTaskClient client, ArchiveRunInformation runInformation, StatusHelper.EntityState state)
        {
            var entityId = GetEntityId();
